...
function validateCampaignRecord(record, tenantId) {
  ...
  if (record.tenantId !== tenantId || record.payload.tenantId !== tenantId || record.recordId !== record.payload.campaignId || record.indexKey !== record.payload.plan.approvalHash) {
...
export function approveDurableReactivationCampaign({ store, tenantId, campaignId, approvalId, approvedBy, approvedPlanHash, approvalAuthorityRef, approvedAt = new Date(), expiresAt = null }) {
  requiredString(approvalAuthorityRef, 'approvalAuthorityRef');
  return transition({ store, tenantId, campaignId, now: approvedAt, apply: campaign => approveReactivationCampaign(campaign, { approvalId, approvedBy, approvedPlanHash, approvalAuthorityRef, approvedAt, expiresAt }), eventExtra: { approvalAuthorityRef } });
}
...
export function startDurableReactivationCampaignFromCommand({ store, tenantId, campaignId, command, now = new Date() }) {
  ...
  return transition({ store, tenantId, campaignId, now, apply: campaign => { assertCommandMatchesCampaign(campaign, command); return startReactivationCampaign(campaign, { attemptId: command.attemptId, now }); }, eventExtra: { commandId: command.commandId, commandHash: command.commandHash, attemptId: command.attemptId, executionBusinessSnapshotId: command.executionBusinessSnapshotId, dispatchMaxRecipients: command.maxRecipients } });
}

export async function startDurableReactivationCampaignFromPersistedCommand({ store, tenantId, campaignId, commandId, now = new Date() }) {
  ...
  if (!commandRecord) throw new Error('DURABLE_WISERR_REACTIVATION_COMMAND_NOT_FOUND');
  const command = clone(commandRecord.payload.command);
  if (command.campaignId !== campaignId) throw new Error('DURABLE_WISERR_COMMAND_CAMPAIGN_MISMATCH');
  return startDurableReactivationCampaignFromCommand({ store, tenantId, campaignId, command, now });
}
