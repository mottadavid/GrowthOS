...
function assertCommandAttemptIdentity(command, attempt) {
  if (command.tenantId !== attempt.tenantId || command.actionId !== attempt.actionId || command.actionHash !== attempt.actionHash || command.attemptId !== attempt.attemptId || command.attemptNumber !== attempt.attemptNumber || command.idempotencyKey !== attempt.idempotencyKey) {
...
function assertExecutingCampaignMatchesCommand(campaign, command) {
  ...
  if (!Array.isArray(campaign.attemptIds) || !campaign.attemptIds.includes(command.attemptId)) throw new Error('WISERR_SUBMISSION_CAMPAIGN_ATTEMPT_MISMATCH');
  if (campaign.plan?.planId !== command.planId || campaign.plan?.approvalHash !== command.planApprovalHash || campaign.approval?.approvalId !== command.campaignApprovalId) {
...
export async function preparePersistedWiserrReactivationSubmission({
  ...
  if (runtime?.tenantId !== tenantId) throw new Error('WISERR_SUBMISSION_RUNTIME_TENANT_MISMATCH');
  ...
  if (attemptRecord.payload.state !== EXECUTION_ATTEMPT_STATES.CREATED) {
    ...
    error.requiresReconciliation = [EXECUTION_ATTEMPT_STATES.SUBMITTING, EXECUTION_ATTEMPT_STATES.ACCEPTED, EXECUTION_ATTEMPT_STATES.RECONCILIATION_REQUIRED].includes(attemptRecord.payload.state);
    throw error;
  }

  if (campaignRecord.payload.status === 'APPROVED') {
    campaignRecord = await startDurableReactivationCampaignFromPersistedCommand({ store, tenantId, campaignId, commandId, now });
  ...
  const submittingAttempt = await markDurableExecutionSubmitting({ store, tenantId, attemptId: command.attemptId, now });
  return { schemaVersion: 1, tenantId, campaignId, commandId, attemptId: command.attemptId, attemptState: submittingAttempt.payload.state, campaignState: campaignRecord.payload.status, submissionAuthorized: true, command };
}
