...
function commandFor(campaignRecord, overrides = {}) {
  ...
  const command = {
    ...
    capacityAuthorityHash: '0'.repeat(64),
...
test('durable approval requires an external authority reference and retains it', async () => {
  ...
  assert.throws(() => approveDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: ready.recordId, approvalId: 'approval-1', approvedBy: 'owner-1', approvedPlanHash: p.approvalHash, approvalAuthorityRef: '', approvedAt: T0 }), /approvalAuthorityRef/);
  const approved = await approveDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: ready.recordId, approvalId: 'approval-1', approvedBy: 'owner-1', approvedPlanHash: p.approvalHash, approvalAuthorityRef: 'wiserr://authority/owner-1/reactivation', approvedAt: T0 });
...
test('valid exact command moves durable approved campaign to executing and remains recoverable', async () => {
  ...
  const executing = await startDurableReactivationCampaignFromCommand({ store, tenantId: 'tenant-1', campaignId: record.recordId, command, now: new Date('2026-08-18T20:03:00Z') });
...
test('self-consistent forged command still cannot change approved campaign semantics', async () => {
  ...
  for (const item of cases) {
    ...
    await assert.rejects(() => startDurableReactivationCampaignFromCommand({ store, tenantId: 'tenant-1', campaignId: record.recordId, command: forged, now: new Date('2026-08-18T20:03:00Z') }), item.error);
...
test('self-consistent command cannot substitute a different execution authority dependency', async () => {
  ...
  const forged = { ...changedBody, commandHash: sha256Canonical(changedBody) };
  assert.throws(() => startDurableReactivationCampaignFromCommand({ store, tenantId: 'tenant-1', campaignId: record.recordId, command: forged, now: new Date('2026-08-18T20:03:00Z') }), /WISERR_REACTIVATION_COMMAND_EXECUTION_AUTHORITY_MISMATCH/);
});
...
test('durable campaign preserves reconciliation and manual stop after restart', async () => {
  ...
  const { record } = await approvedDurableCampaign(store);
  const executing = await startDurableReactivationCampaignFromCommand({ store, tenantId: 'tenant-1', campaignId: record.recordId, command: commandFor(record), now: new Date('2026-08-18T20:03:00Z') });
  const uncertain = await markDurableReactivationCampaignReconciliationRequired({ store, tenantId: 'tenant-1', campaignId: executing.recordId, reason: 'provider outcome unknown', now: new Date('2026-08-18T20:04:00Z') });
  ...
  const stopped = await stopDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: recovered.recordId, reason: 'operator stop until evidence resolved', now: new Date('2026-08-18T20:05:00Z') });
...
test('tampered campaign plan or secondary index fails closed on recovery', async () => {
  ...
  await assert.rejects(() => loadDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: created.record.recordId }), /DURABLE_REACTIVATION_CAMPAIGN_IDENTITY_MISMATCH/);
});
