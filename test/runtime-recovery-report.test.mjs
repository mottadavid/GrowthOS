...
test('terminal tenant runtime state is safe for unattended recovery inspection', async () => {
  ...
  assert.equal(report.mode, 'READ_ONLY'); assert.equal(report.safeForUnattendedRecovery, true); assert.equal(report.requiresHumanOrDeterministicRevalidation, false); assert.equal(report.summary.findingCount, 0);
});

test('suppressed attempt is terminal recovery evidence, not ambiguous work', async () => {
  const store = new AtomicInMemoryRuntimeStore();
...
test('created attempt survives restart as revalidation work and is never auto-resumed', async () => {
  const store = new AtomicInMemoryRuntimeStore(); await put(store, 'execution_attempt', 'attempt-1', { state: 'CREATED', actionId: 'action-1' });
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW });
  assert.equal(report.safeForUnattendedRecovery, false); assert.equal(report.summary.unresolvedAttemptCount, 1); assert.equal(report.findings[0].code, 'ATTEMPT_CREATED_REVALIDATE_BEFORE_SUBMIT');
});
...
test('persisted command with pristine attempt is explicit revalidation work and never auto-submitted', async () => {
  ...
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW });
  assert.equal(report.safeForUnattendedRecovery, false); assert.equal(report.summary.persistedCommandFindingCount, 1); assert.ok(report.findings.some(item => item.code === 'PERSISTED_COMMAND_REVALIDATE_BEFORE_SUBMIT'));
});

test('persisted command without matching durable attempt is blocking', async () => {
  const store = new AtomicInMemoryRuntimeStore(); await put(store, 'wiserr_reactivation_command', 'command-action-1', commandPayload({ attemptId: 'missing-attempt' }));
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW });
...
test('campaign and experiment reconciliation state remains blocking after restart', async () => {
  ...
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW }); assert.equal(report.summary.blockingCount, 2); assert.equal(report.safeForUnattendedRecovery, false);
});
...
test('approved/observing work and expired-but-still-active envelope require deterministic revalidation', async () => {
  ...
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW }); assert.equal(report.summary.blockingCount, 0); assert.equal(report.summary.attentionCount, 3); assert.equal(report.safeForUnattendedRecovery, false);
});
...
test('potentially truncated recovery coverage refuses to declare recovery safe', async () => {
  ...
  const calls = []; const store = { async listRecords(args) { calls.push(args); return args.recordType === 'execution_attempt' ? tenThousandTerminal : []; } };
  const report = await buildTenantRecoveryReport({ store, tenantId: 'tenant-1', now: NOW }); assert.equal(report.safeForUnattendedRecovery, false); assert.deepEqual(report.coverage.potentiallyTruncatedRecordTypes, ['execution_attempt']); assert.equal(calls.every(call => call.tenantId === 'tenant-1' && call.limit === 10000), true);
});
