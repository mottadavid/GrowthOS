...
function buildLoop() {
  ...
  const detected = evaluateDormantLeadReactivation(businessState, { minDormantLeads: 25, now: T0 });
  ...
  const plan = buildReactivationPlan({
    ...
    snapshot: snap,
    channel: 'sms',
    message: { strategy: 'helpful_reactivation', body: 'PRIVATE MESSAGE BODY', version: 'v1' },
  ...
  let experiment = createExperiment({
    ...
    businessSnapshotId: snap.snapshotId,
    hypothesis: 'This bounded reactivation action can generate bookings without breaching opt-out guardrails.',
  ...
  markExecutionCompleted(attempt, { acceptedRecipients: 100 }, new Date('2026-08-18T20:03:00Z'));
  ...
  const outcome = createOutcomeEvent({
    ...
    directCorrelationId: action.actionId,
    occurredAt: new Date('2026-08-18T21:00:00Z')
  });
...
test('manifest refuses cross-tenant, cross-snapshot and cross-action drift', () => {
  const loop = buildLoop();
  assert.throws(() => createGrowthRunManifest({
    runId: 'growth-run-1', snapshot: loop.snap, opportunity: { ...loop.opportunity, tenantId: 'tenant-2' }, campaign: loop.campaign,
    experiment: loop.experiment, envelope: loop.envelope, action: loop.action, attempt: loop.attempt, policyReceipt: loop.policyReceipt, outcomeEvents: [loop.outcome]
  }), /TENANT_MISMATCH/);

  assert.throws(() => createGrowthRunManifest({
    runId: 'growth-run-1', snapshot: loop.snap, opportunity: loop.opportunity, campaign: loop.campaign,
    experiment: { ...loop.experiment, businessSnapshotId: 'other-snapshot' }, envelope: loop.envelope, action: loop.action, attempt: loop.attempt,
    policyReceipt: loop.policyReceipt, outcomeEvents: [loop.outcome]
  }), /EXPERIMENT_SNAPSHOT_MISMATCH/);

  assert.throws(() => createGrowthRunManifest({
    runId: 'growth-run-1', snapshot: loop.snap, opportunity: loop.opportunity, campaign: loop.campaign,
    experiment: loop.experiment, envelope: loop.envelope, action: loop.action, attempt: { ...loop.attempt, actionHash: 'f'.repeat(64) },
    policyReceipt: loop.policyReceipt, outcomeEvents: [loop.outcome]
  }), /ATTEMPT_ACTION_MISMATCH/);
});

test('stored manifest detects later source mutation', () => {
  const loop = buildLoop();
  const input = {
    runId: 'growth-run-1', snapshot: loop.snap, opportunity: loop.opportunity, campaign: loop.campaign,
    experiment: loop.experiment, envelope: loop.envelope, action: loop.action, attempt: loop.attempt,
    policyReceipt: loop.policyReceipt, outcomeEvents: [loop.outcome]
  };
  const manifest = createGrowthRunManifest(input);
  assert.equal(assertGrowthRunManifestMatches(manifest, input), true);
  const changed = { ...input, action: { ...loop.action, geography: 'orlando-fl' } };
  assert.throws(() => assertGrowthRunManifestMatches(manifest, changed), /SOURCE_CHANGED|POLICY_RECEIPT_ACTION_CHANGED/);
});
