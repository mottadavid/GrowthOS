...
function command() {
  const body = {
    ...
    capacityProofHash: 'e'.repeat(64),
    capacitySemanticHash: 'f'.repeat(64),
    capacityAuthorityHash: '1'.repeat(64),
    executionAuthorityDependencyId: WISERR_REACTIVATION_SMS_DEPENDENCY_ID,
    executionAuthorityLockFingerprint: '2'.repeat(64),
    originalBusinessSnapshotId: 'snapshot-original',
    executionBusinessSnapshotId: 'snapshot-execution',
...
async function setup() {
  ...
  return { store, runtime: executionRuntime(store), cmd };
}

test('ACCEPTED result persists exact evidence then advances the durable attempt idempotently', async () => {
  ...
  assert.equal(second.attemptRecord.payload.state, 'ACCEPTED');

  const receipts = await listDurableWiserrSubmissionResults({ store, tenantId: 'tenant-1', attemptId: 'attempt-1' });
  assert.equal(receipts.length, 1);
  assert.equal(receipts[0].payload.result.resultId, inbound.resultId);
});

test('COMPLETED received directly from SUBMITTING records acceptance then completion without losing external identity', async () => {
...
test('ACCEPTED then distinct COMPLETED result progresses the same attempt', async () => {
  const { runtime } = await setup();
...
test('canonical SUPPRESSED result becomes terminal suppression rather than provider failure', async () => {
  const { runtime } = await setup();
  const suppressed = result({
    resultId: 'wiserr-result-suppressed',
    outcome: 'SUPPRESSED',
    classification: 'RECIPIENT_OPTED_OUT',
    evidenceRef: 'wiserr://messaging/suppression/lead-1',
    externalExecutionId: null
  });
  const applied = await ingestWiserrReactivationSubmissionResult({ runtime, result: suppressed, now: NOW });
...
test('AMBIGUOUS result always enters reconciliation-required state', async () => {
  const { runtime } = await setup();
  const ambiguous = result({
    resultId: 'wiserr-result-ambiguous',
    outcome: 'AMBIGUOUS',
    classification: 'TRANSPORT_OUTCOME_UNKNOWN',
    evidenceRef: 'wiserr://messaging/reconciliation/attempt-1',
    externalExecutionId: null
  });
  const applied = await ingestWiserrReactivationSubmissionResult({ runtime, result: ambiguous, now: NOW });
  assert.equal(applied.attemptRecord.payload.state, 'RECONCILIATION_REQUIRED');
});

test('same result ID with changed semantics conflicts before attempt mutation', async () => {
  const { runtime } = await setup();
  const inbound = result();
  await ingestWiserrReactivationSubmissionResult({ runtime, result: inbound, now: NOW });
  const changed = { ...structuredClone(inbound), classification: 'CHANGED_CLASSIFICATION' };
  await assert.rejects(() => ingestWiserrReactivationSubmissionResult({ runtime, result: changed, now: NOW }), /DURABLE_WISERR_RESULT_CONFLICT/);
});

test('cross-command or cross-attempt result is refused before durable receipt creation', async () => {
  const { store, runtime } = await setup();
  const wrong = result({ resultId: 'wrong-result', commandId: 'other-command' });
  await assert.rejects(() => ingestWiserrReactivationSubmissionResult({ runtime, result: wrong, now: NOW }), /WISERR_SUBMISSION_RESULT_COMMAND_NOT_FOUND/);
  const persisted = await loadDurableWiserrSubmissionResult({ store, tenantId: 'tenant-1', resultId: 'wrong-result' });
  assert.equal(persisted, null);
});

test('crash after result receipt persistence but before attempt transition can deterministically reapply the exact result', async () => {
  const { store, runtime } = await setup();
  ...
  const before = await loadDurableExecutionAttempt({ store, tenantId: 'tenant-1', attemptId: 'attempt-1' });
  assert.equal(before.payload.state, 'SUBMITTING');
...
test('result receipt compact event excludes private command message and provider payload fields are rejected', async () => {
  const { store, runtime } = await setup();
  await ingestWiserrReactivationSubmissionResult({ runtime, result: result(), now: NOW });
  ...
  assert.throws(() => result({ providerPayload: { raw: 'secret' } }) && ingestWiserrReactivationSubmissionResult, /$^/);
  assert.throws(() => {
    const invalid = result();
    invalid.providerPayload = { raw: 'secret' };
    // validator runs synchronously before any store access.
    return invalid;
  }, /$^/);
});
