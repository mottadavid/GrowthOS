...
function opportunity() { return { opportunityId: 'opp-1', tenantId: 'tenant-1', businessSnapshotId: 'snapshot-1', type: 'DORMANT_LEAD_REACTIVATION' }; }
function plan() { return buildReactivationPlan({ opportunity: opportunity(), snapshot: snapshot(), channel: 'sms', message: { strategy: 'return-help', body: 'PRIVATE CUSTOMER MESSAGE', version: 'v1' }, requestedMaxRecipients: 50, maxAttempts: 1 }); }
...
async function approvedCampaign(store) {
  ...
  return approveDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: ready.recordId, approvalId: 'approval-1', approvedBy: 'owner-1', approvedPlanHash: p.approvalHash, approvalAuthorityRef: 'wiserr://authority/owner-1/reactivation', approvedAt: NOW });
}
...
function commandFor(campaignRecord, overrides = {}) {
  ...
  const body = {
    schemaVersion: 1, commandId: 'wiserr-reactivation:tenant-1:action-1:attempt:1', tenantId: 'tenant-1', actionId: 'action-1', actionHash: 'a'.repeat(64), campaignId: campaign.campaignId, opportunityId: p.opportunityId, experimentId: 'experiment-1', planId: p.planId, planApprovalHash: p.approvalHash, campaignApprovalId: campaign.approval.approvalId, policyReceiptId: 'policy-1', policyReceiptHash: 'b'.repeat(64), envelopeId: 'envelope-1', envelopeHash: 'c'.repeat(64), attemptId: 'attempt-1', attemptNumber: 1, idempotencyKey: 'growthos:tenant-1:action-1:attempt:1', capacityBundleId: 'capacity-bundle-1', capacityProofHash: 'd'.repeat(64), capacitySemanticHash: 'e'.repeat(64), capacityAuthorityHash: 'f'.repeat(64), executionAuthorityDependencyId: WISERR_REACTIVATION_SMS_DEPENDENCY_ID, executionAuthorityLockFingerprint: '1'.repeat(64), originalBusinessSnapshotId: p.businessSnapshotId, executionBusinessSnapshotId: 'snapshot-execution-1', cohortDefinitionId: p.cohort.definitionId, cohortDefinitionVersion: p.cohort.definitionVersion, channel: p.channel, accountId: 'wiserr-primary', geography: 'tampa-fl', maxRecipients: 35, message: structuredClone(p.message), frequencyPolicy: structuredClone(p.frequencyPolicy), ...overrides
  };
  return { ...body, commandHash: sha256Canonical(body) };
}

test('exact command persists once and exact replay is idempotent', async () => {
  const store = new AtomicInMemoryRuntimeStore(); const campaign = await approvedCampaign(store); const command = commandFor(campaign);
  ...
  assert.equal(first.idempotent, false); assert.equal(second.idempotent, true); assert.equal(first.record.recordId, command.commandId); assert.equal(first.record.indexKey, command.actionId); assert.equal(assertDurableWiserrReactivationCommandMatches(first.record, command), true);
});
...
test('same command ID cannot be reused with changed authority or payload', async () => {
  ...
  const changedBody = { ...structuredClone(command), capacityProofHash: '2'.repeat(64) }; delete changedBody.commandHash;
...
test('action-scoped recovery returns the exact immutable command', async () => {
  ...
  assert.equal(found.length, 1); assert.equal(found[0].payload.command.commandHash, command.commandHash);
});
...
test('compact persistence event never copies private message content', async () => {
  ...
  assert.ok(event); assert.equal(event.payload.commandHash, command.commandHash); assert.equal(JSON.stringify(event).includes('PRIVATE CUSTOMER MESSAGE'), false);
});

test('restart path starts campaign from persisted command ID without caller reconstructing command', async () => {
  const store = new AtomicInMemoryRuntimeStore(); const campaign = await approvedCampaign(store); const command = commandFor(campaign);
  await persistDurableWiserrReactivationCommand({ store, command, now: NOW });
  const started = await startDurableReactivationCampaignFromPersistedCommand({ store, tenantId: 'tenant-1', campaignId: campaign.recordId, commandId: command.commandId, now: new Date('2026-08-18T20:02:00Z') });
  assert.equal(started.payload.status, 'EXECUTING'); assert.deepEqual(started.payload.attemptIds, ['attempt-1']);
  const recovered = await loadDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: campaign.recordId }); assert.equal(recovered.payload.status, 'EXECUTING');
});

test('missing or cross-campaign persisted command refuses before campaign mutation', async () => {
  const store = new AtomicInMemoryRuntimeStore(); const campaign = await approvedCampaign(store);
  ...
  const command = commandFor(campaign, { campaignId: 'other-campaign' }); await persistDurableWiserrReactivationCommand({ store, command, now: NOW });
  ...
  const unchanged = await loadDurableReactivationCampaign({ store, tenantId: 'tenant-1', campaignId: campaign.recordId }); assert.equal(unchanged.payload.status, 'APPROVED');
});
...
test('tampered persisted command record fails closed during recovery', async () => {
  ...
  await assert.rejects(() => loadDurableWiserrReactivationCommand({ store, tenantId: 'tenant-1', commandId: command.commandId }), /RUNTIME_RECORD_HASH_MISMATCH|DURABLE_WISERR_COMMAND_SEMANTIC_HASH_MISMATCH/);
});
